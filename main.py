...
from training import train_encoder_decoder_embeddings, train_encoder_decoder_embeddings_moco, train_encoder_decoder_multidata_embeddings, train_encoder_decoder_embeddings_contrastive, train_encoder_decoder_embeddings_contrastive_kin
from video_preprocessing import computeOpticalFlow, create_data_blobs
from embeddings_cluster_explore import evaluate_model, evaluate_model_superuser, train_and_eval_moco, evaluate_model_multidata, plot_umap_clusters, plot_umap_clusters_multidata
...
def main() -> None:
    ...
    parser.add_argument('--pretrained_encoder_weights', metavar='--pretrained_encoder_weights', type=str)
    parser.add_argument('--noise_variance', metavar='--noise_variance', type=str)
    ...
    if args.mode == 'train':
        ...
        try:
            ...
            lower = blobs_folder_path.lower()
            if 'knot' in lower or 'tying' in lower:
                dataset_name = 'Knot_Tying'
            elif 'needle' in lower or 'passing' in lower:
                dataset_name = 'Needle_Passing'
            elif 'suturing' in lower:
        ...
        train_encoder_decoder_embeddings(net = net, lr = lr, num_epochs = num_epochs, blobs_folder_path = blobs_folder_path, weights_save_path = weights_save_path, weight_decay = weight_decay, dataset_name = dataset_name, model_dim = model_dim)

    elif args.mode == 'train-contrastive':
        weight_decay = 1e-8
        ...
        net = encoderDecoder(embedding_dim = model_dim, return_intermediates = True)

        train_encoder_decoder_embeddings_contrastive(net = net, lr = lr, num_epochs = num_epochs, blobs_folder_path = blobs_folder_path, weights_save_path = weights_save_path, weight_decay = weight_decay, dataset_name = dataset_name, model_dim = model_dim, noise_variance = noise_variance)

    elif args.mode == 'train-contrastive-kin':
        weight_decay = 1e-8
        ...
        train_encoder_decoder_embeddings_contrastive_kin(net = net, lr = lr, num_epochs = num_epochs, blobs_folder_path = blobs_folder_path, weights_save_path = weights_save_path, weight_decay = weight_decay, dataset_name = dataset_name, model_dim = model_dim)

    elif args.mode == 'train-moco':
        weight_decay = 1e-8
        ...
        def pretrained_encoder(embedding_dim):
            enc_dec = encoderDecoder(embedding_dim = embedding_dim)
            enc_dec.load_state_dict(torch.load(pretrained_encoder_weights))
            return enc_dec
            ...
            # return Encoder(enc_dec.conv_net_stream)

        net = MoCo(base_encoder = pretrained_encoder, dim = model_dim)

        train_encoder_decoder_embeddings_moco(net = net, lr = lr, num_epochs = num_epochs, blobs_folder_path = blobs_folder_path, weights_save_path = weights_save_path, weight_decay = weight_decay, dataset_name = dataset_name, model_dim = model_dim)
        ...
        evaluate_model_superuser(blobs_folder_path = blobs_folder_path, model = model, transcriptions_path = transcriptions_folder_path, experimental_setup_path = experimental_setup_path, model_dim = model_dim)
        ...
        plot_umap_clusters(blobs_folder_path = blobs_folder_path, model = model, plot_store_path = 'imgs')
