...
def cluster_statistics(blobs_folder_path: str, model: encoderDecoder, num_clusters: int, *, model_dim: int) -> pd.DataFrame:
    results_dict = store_embeddings_in_dict(blobs_folder_path = blobs_folder_path, model = model)
    k_means = KMeans(n_clusters = num_clusters)
    cluster_indices = k_means.fit_predict(np.array(results_dict['embeddings']).reshape(-1, model_dim))
...
def plot_umap_clusters(blobs_folder_path: str, model: encoderDecoder, plot_store_path: str) -> None:
    ...
    print('Generating skill plots intermediates.')

    with open(os.path.join(plot_store_path, 'umap_skill.npy'), 'wb') as f:
        np.save(f, reduced_embeddings)
        np.save(f, [sns.color_palette()[x] for x in results_dict['skill']])
    ...
    print('Generating gesture plots intermediates.')

    with open(os.path.join(plot_store_path, 'umap_gesture.npy'), 'wb') as f:
        np.save(f, reduced_embeddings)
        np.save(f, [sns.color_palette()[x] for x in le_gest.transform(results_dict['gesture'])])
    ...
    print('Generating user plots intermediates.')

    with open(os.path.join(plot_store_path, 'umap_user.npy'), 'wb') as f:
        np.save(f, reduced_embeddings)
        np.save(f, [sns.color_palette()[x] for x in le_user.transform(results_dict['user'])])
...
def evaluate_model_superuser(blobs_folder_path: str, model: encoderDecoder, transcriptions_path: str, experimental_setup_path: str, model_dim: int) -> None:
    ...
    df = cluster_statistics(blobs_folder_path = blobs_folder_path, model = model, num_clusters = 5, model_dim = model_dim)
    ...
    X = [np.array(v) for v in df['embeddings']]
    X = np.array(X).reshape(-1, model_dim)
...
def train_and_eval_moco(blobs_folder_path: str, model: encoderDecoder, transcriptions_path: str, experimental_setup_path: str, model_dim: int) -> None:
    transcription_file_names = os.listdir(transcriptions_path)
    transcription_file_names = list(filter(lambda x: '.DS_Store' not in x, transcription_file_names))
