...
class encoderDecoder(nn.Module):
    def __init__(self, embedding_dim: int, *, return_intermediates: bool = False) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> Union[torch.Tensor, List[torch.Tensor]]:
        encoded = self.conv_net_stream(x)
        decoded = self.decoder(encoded).view(-1, 25, 1, 76)
        return (encoded, decoded) if self.return_intermediates else decoded
